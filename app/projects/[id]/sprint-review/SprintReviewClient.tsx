...
function splitCompoundItems(items: any[], allFeatures: Feature[]): any[] {
  const result: any[] = []
  for (const item of items) {
    let connector = COMPOUND_CONNECTORS.find(c => item.title.includes(c))
    ...
    // find best matching featureId for part2 (exclude original featureId)
...
export function SprintReviewClient({ projectId, jobId, diffResult, activeSprint, allFeatures = [], commitSha, commitMsg, author, triggeredAt, mockButton, resetButton, noDiff, mockRequirements, readOnly }: Props) {
  ...
  const allReqItems = splitCompoundItems(sprintDocs.flatMap(d => d.items as any[]), allFeatures)
